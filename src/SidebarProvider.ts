...
import {addPort,getAllPorts,onAllPorts,onUrlSet,onUrlSetError,removePort} from './bcTunnel';
function getNonce() {
	let text = '';
...
export class SidebarProvider implements vscode.WebviewViewProvider {
  ...
  private isAssigning:boolean =false;
  private addPortBtnText='Add Port'
  private addPortBtnState=''
  constructor(private readonly _extensionUri: vscode.Uri) {
   setTimeout(()=>{getAllPorts()},2000)
  }
...
//flag that tells loaded or not--->put this flag in updateContentView parameters
  private updateWebviewContent(webview: vscode.Webview,loadState:boolean): void {
    let newHtml='';
    if(loadState)
    {
  ...
  private generateListItems(): string {
    let listItems = '';
